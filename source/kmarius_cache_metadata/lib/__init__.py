...
DB_PATH = os.path.join(common.get_home_dir(), ".unmanic",
                       "userdata", "kmarius_cache_metadata", "cache.db")
...
def init_database(tables: list[str]):
...
def lookup(table: str, path: str, mtime: int = None) -> Optional[dict]:
    ...
    if mtime:
        cur.execute(
            f"SELECT data FROM {table} WHERE path = ? AND mtime = ?", (path, mtime))
    else:
        cur.execute(
            f"SELECT data FROM {table} WHERE path = ?", (path,))
    row = cur.fetchone()
    conn.close()
...
def exists(table: str, path: str, mtime: int = None) -> bool:
    ...
    if mtime:
        [[count]] = cur.execute(
            f"SELECT COUNT(*) FROM {table} WHERE path = ? AND mtime = ? LIMIT 1", (path, mtime))
    else:
        [[count]] = cur.execute(
            f"SELECT COUNT(*) FROM {table} WHERE path = ? LIMIT 1", (path,))
    conn.close()
    return count > 0